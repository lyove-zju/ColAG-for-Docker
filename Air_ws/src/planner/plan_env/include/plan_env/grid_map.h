...
struct MappingData {
  ...
  std::map<double,Eigen::Vector3d> camera_pos_win_;
  std::vector<OneIDodomOfSwarm> ugv_odom_buf_;
   std::vector<pcl::PointCloud<pcl::PointXYZ> > send_cloud_;
  double now_map_stamp_;
  // depth image data
...
public:
  ...
  inline bool isBoundUGV(const Eigen::Vector3d& pos);
  inline bool isBoundUGV(const pcl::PointXYZ & pt);
...
private:
  ...
  void lidarCloudCallback(const sensor_msgs::PointCloud2ConstPtr &img);
  void lidarOdomCallback(const nav_msgs::OdometryConstPtr &img);

  void odomCallback(const nav_msgs::OdometryConstPtr& odom);
  // void BroadcastOdomCallback (const  fake_car::IDodomPtr & msg);
  // void BroadcastOdomCallback (const  nav_msgs::OdometryConstPtr & msg);
  void UGVOdomCallback(const nav_msgs::OdometryConstPtr &odom);
  ...
  void inflateCount(const Eigen::Vector3i idx, const bool isup);
  
  inline void inflatePoint(const Eigen::Vector3i& pt, int step, vector<Eigen::Vector3i>& pts);

  int setCacheOccupancy(Eigen::Vector3d pos, int occ);
  Eigen::Vector3d closetPointInMap(const Eigen::Vector3d& pt, const Eigen::Vector3d& camera_pt);
  Eigen::Vector3d closetPointFromUGV (const Eigen::Vector3d& pt, const Eigen::Vector3d& camera_pt);
...
inline int GridMap::getInflateOccupancy(Eigen::Vector3d pos) {
  ...
  return int(md_.occupancy_buffer_inflate_[toAddress(id)] >= 1);
}
...
inline bool GridMap::isBoundUGV(const Eigen::Vector3d& pos) {
  ...
  //
    ...
    double dy = pos(1) - md_.ugv_odom_buf_[i].pos.y();
    double dz = pos(2) - md_.ugv_odom_buf_[i].pos.z();
    if (fabs(dx) <= ugvbox_x_ && fabs(dy) <= ugvbox_y_ && fabs(dz) <= ugvbox_z_)
    {
       return true;
    }
   }
...
inline bool GridMap::isBoundUGV(const pcl::PointXYZ & pt) {
  bool surface_flag = false;
  for(int i = 0; i < md_.ugv_odom_buf_.size(); i++)
  {
    if(md_.ugv_odom_buf_[i].drone_id == -1) continue;
  //
    double dx = pt.x - md_.ugv_odom_buf_[i].pos.x();
    double dy = pt.y - md_.ugv_odom_buf_[i].pos.y();
    double dz = pt.z - md_.ugv_odom_buf_[i].pos.z();
    if (fabs(dx) <= ugvbox_x_ && fabs(dy) <= ugvbox_y_ && fabs(dz) <= ugvbox_z_)
    {
        surface_flag = true;
    }
    else if (fabs(dx) <= mp_.send_update_range_(0) && fabs(dy) <= mp_.send_update_range_ (1) && fabs(dz) <= mp_.send_update_range_(2))
    {
        md_.send_cloud_[i].push_back(pt);
    }
  }
   return surface_flag;
}
